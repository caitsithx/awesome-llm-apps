...
The **Breakup Recovery Squad** is designed as a multi-agent application using **Streamlit** for the frontend interface and the **Agno** framework for agent orchestration, powered by **Anthropic's Claude model** via a configurable AI gateway.
...
*   **Auth Display**: Shows authentication status, token expiry countdown, and troubleshooting help. No manual API key entry—uses cached JWT from configured token file.
...
Standalone agents orchestrated with `asyncio` sharing a model configuration ([Claude](file:///Users/xili/Documents/codes/github/awesome-llm-apps/starter_ai_agents/ai_breakup_recovery_agent/ai_breakup_recovery_agent.py#L42-47)):
*   **Relationship Recovery Coordinator**: Receives outputs from all 4 specialist agents, synthesizes them into a cohesive recovery roadmap with unified voice.
...
### 3. AI Gateway Layer
*   **JWT Token Management** ([get_jwt_token](file:///Users/xili/Documents/codes/github/awesome-llm-apps/starter_ai_agents/ai_breakup_recovery_agent/ai_breakup_recovery_agent.py#L17-38)): Manages authentication tokens cached locally. Auto-refreshes if expired (24h TTL). Configured via `MY_TOKEN_FILE` and `MY_TOKEN_REFRESH_SCRIPT` env vars.
*   **Claude Model via Configurable Endpoint** ([Claude + client_params](file:///Users/xili/Documents/codes/github/awesome-llm-apps/starter_ai_agents/ai_breakup_recovery_agent/ai_breakup_recovery_agent.py#L42-47)): Routes requests to configured LLM endpoint (`MY_LLM_ENDPOINT`) with Bearer token auth. Uses configured model alias (`MY_LLM_MODEL`).
